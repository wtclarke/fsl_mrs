...
def test_FSLModel_vars():
    for model, ans_n, ans_s in zip(all_models, answer_names, answer_sizes):
        names, sizes = models.FSLModel_vars(model, n_basis, n_groups, b_order)
        assert names == ans_n
        assert sizes == ans_s
...
def test_FSLModel_bounds():
    for model, ans_s in zip(all_models, answer_sizes):
        # Newton
        bounds = models.FSLModel_bounds(model, n_basis, n_groups, b_order, 'Newton')
        assert len(bounds) == sum(ans_s)

        # MH
        LB, UB = models.FSLModel_bounds(model, n_basis, n_groups, b_order, 'MH')
...
def test_FSLModel_mask():
    for model, ans_s in zip(all_models, answer_sizes):
        mask = models.FSLModel_mask(model, n_basis, n_groups, b_order)

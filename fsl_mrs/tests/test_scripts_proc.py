...
def svs_data_diff(tmp_path):    
    ...
    FID2,hdr = syntheticFID(noisecovariance = noiseconv,coilamps=coilamps,coilphase=coilphs)
...
def test_coilcombine(svs_data_uncomb,mrsi_data_uncomb,tmp_path):
    ...
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','coilcombine','--file',svsfile])
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','coilcombine','--file',mrsifile])
...
def test_average(svs_data,mrsi_data,tmp_path):
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','average','--file',svsfile[0],svsfile[1],svsfile[2],'--avgfiles'])
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','average','--file',mrsifile[0],mrsifile[1],mrsifile[2],'--avgfiles'])
...
def test_align(svs_data,mrsi_data,tmp_path):
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','align','--file',svsfile[0],svsfile[1],svsfile[2],'--ppm','-10','10'])
    ...
    directRun,_,_ = preproc.phase_freq_align(allFileData,4000,123E6,niter=2,ppmlim=[-10.0,10.0],verbose=False,target=None)

    assert np.isclose(data,directRun[0]).all()

    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','align','--file',mrsifile[0],mrsifile[1],mrsifile[2],'--ppm','-10','10'])
...
def test_ecc(svs_data,mrsi_data,tmp_path):
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','ecc','--file',svsfile[0],'--reference',svsfile[1]])
    ...
    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','ecc','--file',mrsifile[0],'--reference',mrsifile[1]])
...
def test_remove(svs_data,mrsi_data,tmp_path):
    ...
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','remove','--file',svsfile[0],'--ppm','-10','10'])
    ...
    # Run using preproc.py directly
    limits = (-10,10)
    directRun = preproc.hlsvd(svsdata[0],1/4000,123.0,limits,limitUnits='ppm+shift')

    assert np.isclose(data,directRun).all()

    # Run coil combination on both sets of data using the command line
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','remove','--file',mrsifile[0],'--ppm','-10','10'])
    ...
    # Run using preproc.py directly
    limits = (-10,10)
    directRun = preproc.hlsvd(mrsidata[0][2,2,2,...],1/4000,123.0,limits,limitUnits='ppm+shift')

    assert np.isclose(data[2,2,2,...],directRun).all()

def test_align_diff(svs_data_diff,mrsi_data_diff,tmp_path):
    svsfile1,svsfile2,svsdata1,svsdata2 = svs_data_diff[0],svs_data_diff[1],svs_data_diff[2],svs_data_diff[3]
    mrsifile1,mrsifile2,mrsidata1,mrsidata2 = mrsi_data_diff[0],mrsi_data_diff[1],mrsi_data_diff[2],mrsi_data_diff[3]

    # Run alignment via commandline
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp','align-diff',
    ...
    # Run using preproc.py directly    
    directRun,_,_,_ = preproc.phase_freq_align_diff(svsdata1,svsdata2,4000,123E6,ppmlim=[-10.0,10.0])
...
def test_fshift(svs_data,mrsi_data,tmp_path):
    ...
    subprocess.check_call(['fsl_mrs_proc','--output',tmp_path,'--filename','tmp',
                            'fshift',
                            '--file',svsfile[0],
                            '--shiftppm','1.0'])
    ...
    # Run using preproc.py directly
    directRun,_ = preproc.shiftToRef(svsdata[0],4.0,4000.0,123E6,ppmlim=(-5.0,5.0))

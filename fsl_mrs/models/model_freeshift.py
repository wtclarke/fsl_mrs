...
def mask(
    ...
    :param fit_shape: Whether to fit the lineshapes (inc shifts), defaults to True
...
def grad(x, nu, t, m, B, G, g, data, indices: slice):
    ...
    resid = S - Spec
    grad = np.empty(2 * n + 2 * g + 2 + dSdb.shape[1], dtype=float)
    ...
    offset += g
    grad[offset:offset + n] = 2 * np.real(np.sum(resid * np.conj(dSdeps), axis=0))
    offset += n

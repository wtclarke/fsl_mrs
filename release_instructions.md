...
9. For new version create a new release, using the next "x.x.x" tag number as name and release title. New changelog contents are copied into release notes.
10. This will trigger doc build (published [here](https://open.oxcin.ox.ac.uk/pages/fsl/fsl_mrs/)) and package build.
11. Trigger the package build by accepting the relevant merge request on [fsl/conda/fsl_mrs](https://git.fmrib.ox.ac.uk/fsl/conda/fsl_mrs) branch. This will trigger a pipeline from which the conda package be released. Do this by manually triggering the `deploy-all-conda-packages` job after the build has run.
12. New packages will be published to the [FSL conda channel](https://fsl.fmrib.ox.ac.uk/fsldownloads/fslconda/public/).
13. To request a version to be included in the next FSL release, either email Paul/Matthew or open a MR for the development team on the [manifest repository](https://git.fmrib.ox.ac.uk/fsl/conda/manifest/).
